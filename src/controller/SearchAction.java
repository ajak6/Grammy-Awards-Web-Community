	...
	public String perform(HttpServletRequest request) {
		try {
			...
			History historyChart[] = historyDAO.match(MatchArg.and(
					MatchArg.equals("fullName", tag),
					MatchArg.greaterThanOrEqualTo("searchDate",
							calStart.getTime())));
			// sort history list
			...
			Celebrity celebrityTag[] = celebrityDAO.match(MatchArg.equals(
					"fullName", tag));
			int totalTag = 0;
			...
			for (Celebrity celebrity : celebrityTotal) {
				ar.add(celebrity);
			}
			System.out.println(ar.get(0).getFullName());
			Collections.sort(ar, new SearchCompare());
			System.out.println();
			System.out.println("after sort "+ar.get(0).getFullName());
			ServletContext context=request.getSession().getServletContext();
			ArrayList<Celebrity> top4=new ArrayList<Celebrity>();
			for(int i=0;i<4;i++){
				top4.add(ar.get(i));
			}
			context.setAttribute("trends",top4);
...
class SearchCompare implements Comparator<Celebrity>{

	@Override
	public int compare(Celebrity o1, Celebrity o2) {
		return o2.getSearchNum()-o1.getSearchNum();
	}
